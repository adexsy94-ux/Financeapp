...
def create_company_and_admin(
    ...
    log_action(admin_username_norm, "create_company_and_admin", "companies", ref=str(company_id),
...
def update_user_permissions(
    ...
    try:
        ...
        log_action(
            ...
            "users",
            ref=str(user_id),
            details=f"company_id={company_id}, role={role}, create={can_create_voucher}, "
                    f"approve={can_approve_voucher}, manage_users={can_manage_users}",
        )
        return None
    except Exception as e:
...
def change_password(company_id: int, username: str, old_password: str, new_password: str) -> Optional[str]:
    ...
    with closing(connect()) as conn, closing(conn.cursor()) as cur:
        cur.execute(
            """
            SELECT u.id, u.password_hash, c.code AS company_code
            FROM users u
            JOIN companies c ON u.company_id = c.id
...
def require_login():
    ...
    with tab_login:
        ...
        if st.button("Login"):
            user_obj = verify_user(company_code, username, password)
            if not user_obj:
    ...
    with tab_register:
        st.info("Use this form to register a new company and its first admin.")
        company_name = st.text_input("Company Name", key="reg_company_name")
        company_code = st.text_input(
            "Company Code (short, unique – e.g. JAGA, ZETACOMS)",
            key="reg_company_code",
        )
        admin_username = st.text_input("Admin Username", key="reg_admin_username")
        pw1 = st.text_input("Admin Password", type="password", key="reg_admin_pw1")
        pw2 = st.text_input("Confirm Admin Password", type="password", key="reg_admin_pw2")
        if st.button("Register Company"):
            if not company_name or not company_code or not admin_username or not pw1 or not pw2:
                ...
                if err:
                    ...
                    st.success("Company registered and admin created. Please go to Login tab and sign in.")
